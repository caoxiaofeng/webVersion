...
//         }
    ...
    function StrokeBoxedText2($x, $y, $txt, $dir = 0, $fcolor = "white", $bcolor = "black", $shadowcolor = false, $paragraph_align = "left", $xmarg = 6, $ymarg = 4, $cornerradius = 0, $dropwidth = 3)
    {
    ...
    function DashedLineForGrid($x1, $y1, $x2, $y2, $dash_length = 1, $dash_space = 4)
    {
        ...
        /*
         * $dash_length *= $this->scale; $dash_space *= $this->scale;
         */
    ...
    function DrawLine($im, $x1, $y1, $x2, $y2, $weight, $color)
    {
        ...
        if ($y2 - $y1 == 0) {
            ...
            /*
             * $lean = ($y2 - $y1) / ($x2 - $x1); $lean2 = -1 / $lean; $sin = $lean / ($y2 - $y1); $cos = $lean / ($x2 - $x1); $pts[] = $x1 + (-$weight * $sin); $pts[] = $y1 + (-$weight * $cos); $pts[] = $x1 + (+$weight * $sin); $pts[] = $y1 + (+$weight * $cos); $pts[] = $x2 + (+$weight * $sin); $pts[] = $y2 + (+$weight * $cos); $pts[] = $x2 + (-$weight * $sin); $pts[] = $y2 + (-$weight * $cos);
             */
        }
