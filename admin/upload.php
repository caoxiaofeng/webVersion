...
if (isset($_FILES['up'])) {
    if (is_uploaded_file($_FILES['up']['tmp_name'])) {
        if ($up_type == 'pic') {
            ...
            $value_arr = up_img($_FILES['up'], $is_up_size, array(
                'image/gif',
                'image/jpeg',
                'image/png',
                'image/jpg',
                'image/bmp',
                'image/pjpeg'
            ), $is_thumb, $thumb_width, $thumb_height, $logo);
            $pic = $value_arr['pic'];
...
<form name="up" action="" method="post" enctype="multipart/form-data">
	...
	<?php }?>
	<input type="file" name="up" /><?php if($up_type=='pic'){?>&nbsp;&nbsp;<input
			type="checkbox" value="1" <?php if($lock){?> checked="checked"
			<?php }?> name="thumb" style="margin: 0 5px;" />缩略图&nbsp;&nbsp;宽<input
			name="thumb_width" value="<?php echo $_sys['thump_width'];?>"
			style="margin: 0 5px; width: 40px;" />px&nbsp;&nbsp;高<input
			name="thumb_height" style="margin: 0 5px; width: 40px;"
			value="<?php echo $_sys['thump_height'];?>" />px<?php }?>&nbsp;&nbsp;&nbsp;<input
