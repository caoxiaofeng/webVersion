...
<form name="up" action="" method="post" enctype="multipart/form-data">
		...
		<div id="pic_contain">
			<div class="pic">
				<p>
					<input type="file" name="up" />&nbsp;文件说明：<input type="text"
						name="file_info" />
				</p>
			</div>
		</div>
	...
	<!--图片列表-->
	<div class="sl_pic">
		<input type="button" name="sl_pic" id="sl" class="wBox_close"
			value="确定选择" /><span style="padding-left: 15px;">选择好文件后，确定按钮</span>
	</div>
<?php
...
if (! empty($rel)) {
 ...
 <div class="order_main">
    ...
    foreach ($rel as $k => $v) {
        ?>
	<tr>
					<td style="width: 5%; text-align: center"><input name="sl_rd"
						id="file_rd" type="radio"
						<?php if(!$k){echo 'checked="checked"';}?>
						value="<?php echo '../'.$v['file_path'].'|||'.($v['file_size']/1000);?>" /></td>
					<td style="width: 10%; text-align: center" align="center"><?php echo $v['id'];?></td>
					<td style="width: 25%; text-align: center" align="center"><?php echo ($v['file_size']/1000).'KB';?></td>
					<td scope="width:25%;text-align:center" align="center"><?php echo $v['file_path'];?></td>
					<td style="width: 15%; text-align: center"><?php echo $v['file_ext'];?></td>
					<td style="width: 15%; text-align: center"><?php echo date('Y-m-d H:m:s',$v['file_time']);?></td>
				</tr>
<?php
    }
    ?>
