...
elseif ($action == 'save_add') {
    ...
    if (empty($market_name)) {
        msg('<span style="color:red">客服名称不能为空</span>');
    }
    if (empty($market_type)) {
        msg('<span style="color:red">请选择客服类型</span>');
    }
    if (empty($market_num)) {
        msg('<span style="color:red">客服号码不能为空</span>');
    }
    if (strlen($market_name) > 60 || strlen($market_num) > 60) {
        msg('<span style="color:red">添加写内容过长，请缩短填写</span>');
    }
    if (empty($market_order)) {
        msg('<span style="color:red">排序不能为空</span>');
    }
