...
.pic_list_ct li.on {
	border: 2px solid #ff8215;
}

.pic_list_ct li .pic_cz {
	height: 15px;
	background: #fff;
	width: 100%;
	position: absolute;
...
.pic_list_ct li a {
	...
	margin-left: 5px;
	display: inline;
	background: url(template/images/c_alt.gif) no-repeat left top;
}
...
.pic_list_ct li em {
	...
	width: 9px;
	height: 9px;
	background: url(template/images/input_ok.gif) no-repeat right bottom;
	position: absolute;
	right: 1px;
	bottom: 1px;
}

.pic_list li_ct label {
	display: block;
	height: 20px;
	line-height: 20px;
}
    ...
    if (is_array($_FILES['up']['tmp_name'])) {
        foreach ($_FILES['up']['tmp_name'] as $k => $v) {
            ...
            if (is_uploaded_file($v)) {
                ...
                $value_arr = up_img($pic_info, $is_up_size, array(
                    'image/gif',
                    'image/jpeg',
                    'image/png',
                    'image/jpg',
                    ...
                    'image/pjpeg',
                    'image/x-png'
                ), $up_is_thumb, $up_thumb_width, $up_thumb_height, $logo = 1);
                // 处理上传后的图片信息
	...
	<div class="up_contain">
		...
		<div class="xc_contain">
			...
			<div class="sl_pic_thumb">
				<input type="checkbox" value="1" name="is_thumb" id="is_pic_thumb" /><label
					for="is_pic_thumb">&nbsp;缩略图&nbsp;(勾选该选项将会优先使用缩略图)</label>
			</div>
			<!--图片分类-->
...
if (! empty($cate_rel)) {
    foreach ($cate_rel as $nav_k => $nav_v) {
        ...
        ?>
	<li><a class="<?php if($nav_v['id']==$pic_nav){echo 'hover';}?>"
						href="?<?php echo $query;?>"><?php echo $nav_v['cate_name'];?></a></li>
<?php
    }
}
?>
...
<?php
    foreach ($rel as $k => $v) {
        ...
        ?>
<li><img src="<?php echo $img_thumb;?>" border="0" width="80"
					height="80"
					title="【图片名称】：<?php echo $v['pic_name']?> 
【图片说明】：<?php echo $v['pic_alt']?> 
【上传时间】：<?php echo date('Y-m-d H:m:s',$v['pic_time']);?> 
【格式】：<?php echo $v['pic_ext'];?> "
					alt="【图片名称】：<?php echo $v['pic_name']?> 
【图片说明】：<?php echo $v['pic_alt']?> 
【上传时间】：<?php echo date('Y-m-d H:m:s',$v['pic_time']);?> 
【格式】：<?php echo $v['pic_ext'];?> " /> <label><input
						style="display: none" <?php if($type=='radio'){?> type="radio"
						<?php }elseif($type=='checkbox'){?> type="checkbox" <?php }?>
						rel="<?php echo $v['id'];?>" id="pic_sl"
						value="<?php echo str_replace('upload/','',$pic);?>" name="pic_sl" /><input
						type="hidden" name="alt" id="alt"
						value="<?php echo $v['pic_alt'];?>" /><input type="hidden"
						id="pic_val" value="<?php echo str_replace('upload/','',$pic);?>" /><input
						type="hidden" id="pic_thumb" name="pic_thumb"
						value="<?php echo str_replace('upload/','',$v['pic_thumb']);?>" /></label>
					<div class="pic_cz" style="display: none">
						<a title="查看" class="xg_link" href="<?php echo '../'.$pic;?>"
							target="_blank"></a>
					</div> <em style="display: none"></em></li>
<?php
    }
    ?>
</ul>
...
<div class="page page_fy" style="clear: both">
		...
		<?php echo page('admin_pic_upload.php',$page,$query,$total_num,$total_page);?>
			...
			<div class="ok_btn">
				<input type="button" class="wBox_close" name="sl_pic"
					<?php if($type=='radio'){?> id="sl"
					<?php }elseif($type=='checkbox'){?> id="sl_ck" <?php }?> value="确定" />
			</div>
		...
		<div class="xc_contain" style="display: none">
			...
			<form name="up" action="" method="post" enctype="multipart/form-data">
				<div class="pic_fl">
					<label>选择分类：</label><?php echo $cate_str;?></div>
				<div class="pic_fl">
					<p>
						<label>缩略图：</label><label style="width: 30px; padding-top: 10px;"><input
							type="checkbox" onclick="check_is_thumb(this);" value="1"
							name="thumb" style="margin: 0 5px;" checked="checked" /><input
							type="hidden" name="is_thumb" value="1" id="is_thumb" /></label>
					</p>
					<p>
						&nbsp;&nbsp;宽<input name="thumb_width" id="thumb_width"
							value="<?php echo $_sys['thump_width'];?>"
							style="margin: 0 5px; width: 30px;" />px&nbsp;&nbsp;高<input
							name="thumb_height" id="thumb_height"
							style="margin: 0 5px; width: 30px;"
							value="<?php echo $_sys['thump_height'];?>" />px
					</p>
				</div>
				<div class="pic_fl">
					<label>增加上传图片：</label><input name="num" value="3" id="num"
						style="width: 30px; padding: 2px 0;" />&nbsp;&nbsp;&nbsp;(最多5张图片同时上传!允许上传的图片类型:<?php echo $type_pic;?>)</div>
				<div class="pic_f1">
					<input type="button" id="add_num"
						style="float: none; margin-left: 110px;" class="go" value="增加" />
				</div>
				<div id="pic_contain">
					<div class="pic">
						<p>
							<input type="file" name="up[]" />&nbsp;图片说明(alt)：<input
								type="text" name="pic_alt[]" style="width: 100px;" />
						</p>
					</div>
				</div>
				<p style="margin-top: 10px">
					<input type="submit" value="上传" name="uppic" />
				</p>
			</form>
