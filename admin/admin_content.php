...
// 添加内容界面
elseif ($action == 'add') {
    if (! check_purview('content_create')) {
...
elseif ($action == 'save_content') {
    ...
    if (empty($thumb)) {
        ...
        unset($html_pic_arr);
        if (! empty($html_pic)) { // 存在图片取第一张做缩略图
...
elseif ($action == 'save_edit_content') {
    ...
    /*
     * //判断表是否存在 $tables=$GLOBALS['mysql']->show_tables(); if(!in_array($table,$tables)){ msg('没有当前频道的数据表,请重新添加频道'); }
     */
    $filter_str = '';
    if ($filter) {
        foreach ($filter as $key => $value) {
...
elseif ($action == 'del') {
    ...
    unset($channel);
    if (empty($table)) {
        msg('<span style="color:red">不存在相关频道,请更新频道缓存</span>');
    }
...
elseif ($action == 'del_all') {
    ...
    if ($step == '2') {
        // 初始化
        if (empty($all)) {
            msg('<span style="color:red">请选择需要删除的文章</span>');
        }
...
elseif ($action == 'verify') {
    if (! check_purview('content_verify')) {
        msg('<span style="color:red">操作失败,你的权限不足!</span>');
    }
    $all = $_REQUEST['all'];
    $channel_id = intval($_REQUEST['channel_id']);
    ...
    if ($step == '2') {
        if (empty($all)) {
            msg('<span style="color:red">请选择需要审核的内容</span>');
        }
        $str = "<?php\n\$arr=" . var_export($all, true) . ";\n?>";
        cache_write(DATA_PATH . 'cache_arr/content_arr.php', $str);
...
elseif ($action == 'save_move') {
    ...
    if (empty($move_cate)) {
        msg('<span style="color:red">请选择需要转移的栏目</span>');
    }
    if (empty($arc_id) || empty($channel)) {
        msg('<span style="color:red">参数错误,请重新操作</span>', '?action=content_list&lang=' . $lang);
    }
    $id_arr = array();
...
elseif ($action == 'order') {
    ...
    if (empty($order) || empty($order_id)) {
        msg('参数发生错误！请重新操作！', '?action=content_list&lang=' . $lang);
    }
