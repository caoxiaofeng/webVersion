...
if ($action == 'made') {
    ...
    $book_pos = (isset($rel[0]['book_pos']) && ! empty($rel[0]['book_pos'])) ? explode(',', $rel[0]['book_pos']) : array(
        '0'
    );
...
elseif ($action == 'verify') {
    $all = $_POST['all'];
    if (empty($all)) {
        die("<script type=\"text/javascript\">alert('请选择需要审核的内容');history.go(-1);</script>");
    }
    foreach ($all as $k => $v) {
...
elseif ($action == 'reply') {
    ...
    if (empty($id)) {
        die("<script type=\"text/javascript\">alert('参数发生错误，请重新操作');history.go(-1);</script>");
    }
    ...
    if (! empty($rel[0]['pr_id'])) {
        $pr_rel = $mysql->fetch_asc("select title from " . DB_PRE . "maintb where id=" . $rel[0]['pr_id']);
    }
    ;
...
elseif ($action == 'save_reply') {
    ...
    $reply = $_POST['reply'];
    if (empty($reply)) {
        die("<script type=\"text/javascript\">alert('回复内容不能为空');history.go(-1);</script>");
    }
