...
elseif ($action == 'save_admin_edit') {
    ...
    if (strlen($admin_password) > 60) {
        msg('<span style="color:red">新密码太长,请缩短</span>');
    }
    if (strlen($admin_nich) > 60) {
        msg('<span style="color:red">用户昵称太长,请缩短</span>');
    }
    if (strlen($admin_admin) > 60) {
        msg('<span style="color:red">用户姓名太长,请缩短</span>');
    }
    if (strlen($admin_mail) > 60) {
        msg('<span style="color:red">电子邮箱太长,请缩短</span>');
    }
    if (strlen($admin_tel) > 60) {
        msg('<span style="color:red">联系电话太长,请缩短</span>');
    }
    ...
    if ($rel_purview == 1) {
        if ($purview != 1) {
            ...
            if ($admin_num == 1) {
                err('<span style="color:red">请先添加一个超级管理员，才能修改该管理员</span>');
            }
        }
    }
    $table = DB_PRE . "admin";
    ...
    if (! empty($admin_password) || ! empty($admin_password_tow)) {
        if ($admin_password != $admin_password_tow) {
            msg('<span style="color:red">输入的两次新密码不一样</span>');
        }
        if ($password != md5($admin_password_ago)) {
            msg('<span style="color:red">输入的旧密码不正确</span>');
        }
...
elseif ($action == 'save_admin') {
    ...
    if (strlen($admin_name) > 60) {
        msg('<span style="color:red">登录用户名太长,请缩短</span>');
    }
    if (strlen($admin_password) > 60) {
        msg('<span style="color:red">登录用户密码太长,请缩短</span>');
    }
    if (strlen($admin_nich) > 60) {
        msg('<span style="color:red">用户昵称太长,请缩短</span>');
    }
    ...
    if ($admin_password != $admin_password2) {
        msg('<span style="color:red">两次密码不一样</span>');
    }
    if (empty($admin_nich)) {
...
elseif ($action == 'admin_del') {
    ...
    if ($purview == 1) {
        $admin_num = $GLOBALS['mysql']->fetch_rows("select id from " . DB_PRE . "admin where admin_purview=1");
        if ($admin_num == 1) {
            err('<span style="color:red">请先添加一个超级管理员</span>');
        }
    }
    if ($user == $_SESSION['admin']) {
        msg('<span style="color:red">不能删除正在使用的管理员【' . $_SESSION['admin'] . '】</span>');
    }
...
elseif ($action == 'save_admingroup') {
    ...
    if (strlen($admin_group_name) > 60) {
        msg('<span style="color:red">管理组名称太长,请缩短</span>');
    }
    if (strlen($admin_group_info) > 60) {
        msg('<span style="color:red">管理组描述太长,请缩短</span>');
    }
