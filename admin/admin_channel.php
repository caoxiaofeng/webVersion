...
elseif ($action == 'save_channel') {
    ...
    if (strlen($channel_name) > 60) {
        msg('<span style="color:red">模型名太长,请缩短</span>');
    }
    if (! check_str($channel_mark, '/^\w+$/')) {
        msg('<span style="color:red">模型标识只能是字母、数字或_组合</span>');
    }
    if (strlen($channel_mark) > 60) {
        msg('<span style="color:red">模型标识太长,请缩短</span>');
    }
    if (! isset($channel_table) || empty($channel_table)) {
        msg('<span style="color:red">数据表不能为空</span>');
    }
    if (strlen($channel_table) > 60) {
        msg('<span style="color:red">表名太长,请缩短</span>');
    }
    ...
    if (strlen($channel_order) > 60) {
        msg('<span style="color:red">排序数字太长,请缩短</span>');
    }
...
elseif ($action == 'del_channel') {
    ...
    // 取出栏目
    if ($step == 2) {
        if (file_exists(DATA_PATH . 'cache_channel/cate_arr.php')) {
            include (DATA_PATH . 'cache_channel/cate_arr.php');
        }
...
elseif ($action == 'save_field') {
    ...
    /*
     * if(strlen($field_name)>60){msg('<span style="color:red">字段名太长,请缩短</span>');} if(strlen($field_value)>200){msg('<span style="color:red">字段默认值太长,请缩短</span>');} if(strlen($field_info)>200){msg('<span style="color:red">字段说明太长,请缩短</span>');}
     */
    ...
    if (strlen($field_order) > 60) {
        msg('<span style="color:red">字段排序数字太长,请缩短</span>');
    }
    if (file_exists(DATA_PATH . "cache_channel/cache_channel_all.php")) {
        include (DATA_PATH . "cache_channel/cache_channel_all.php");
    }
