...
elseif ($action == 'save_form') {
    ...
    if (strlen($form_name) > 60) {
        msg('<span style="color:red">模型名称太长,请缩短</span>');
    }
    if (strlen($form_mark) > 8) {
        msg('<span style="color:red">模型标识超过10个字数，请缩短</span>');
    }
...
elseif ($action == 'form_xg') {
    ...
    if (empty($form)) {
        msg('<span style="color:red">请先更新缓存</span>');
    }
    foreach ($form as $k => $v) {
        if ($id == $v['id']) {
            $rel = $v;
...
elseif ($action == 'add_field') {
    if (! check_purview('form_field_create')) {
        msg('<span style="color:red">操作失败,你的权限不足!</span>');
    }
    $id = intval($_GET['id']);
...
elseif ($action == 'save_field') {
    ...
    $is_disable = intval($_POST['is_disable']);
    $id = intval($_POST['id']);
    if (empty($use_name) || empty($field_name)) {
        msg('<span style="color:red">提示文字或字段名称不能为空</span>');
    }
    if (strlen($use_name) > 60) {
        msg('<span style="color:red">提示文字长度太长,请缩短</span>');
    }
    if (strlen($field_name) > 15) {
        msg('<span style="color:red">字段名称不能超过15个字符,请缩短</span>');
    }
    if (strlen($field_value) > 200) {
        msg('<span style="color:red">字段类型太长,请缩短</span>');
    }
    if (strlen($field_info) > 200) {
        msg('<span style="color:red">字段说明太长,请缩短</span>');
    }
    if (strlen($form_order) > 200) {
        msg('<span style="color:red">字段排序字数太长,请缩短</span>');
    }
...
elseif ($action == 'xg_field') {
    if (! check_purview('form_field_edit')) {
        msg('<span style="color:red">操作失败,你的权限不足!</span>');
    }
    $id = intval($_GET['id']);
...
elseif ($action == 'import') {
    if (! check_purview('form_field_del')) {
        msg('<span style="color:red">操作失败,你的权限不足!</span>');
    }
...
elseif ($action == 'del_field') {
    ...
    if (empty($id) || empty($form_id)) {
        msg('<span style="color:red">不存在该字段,删除失败</span>');
    }
    // 删除表子字段
    if (file_exists(DATA_PATH . 'cache_form/form.php')) {
        include (DATA_PATH . 'cache_form/form.php');
...
elseif ($action == 'show_form') {
    ...
    $form_id = intval($_GET['form_id']);
    if (empty($id)) {
        msg('<span style="color:red">不存在当前表单</span>');
    }
    if (file_exists(DATA_PATH . 'cache_form/form.php')) {
        include (DATA_PATH . 'cache_form/form.php');
    }
    ...
    if (empty($table)) {
        err('<span style="color:red">不存在表单模型</span>');
    }
...
elseif ($action == 'del_list') {
    ...
    $form_id = intval($_GET['form_id']);
    if (empty($id) || empty($form_id)) {
        msg('<span style="color:red">参数发生错误,删除失败</span>');
    }
    if (file_exists(DATA_PATH . 'cache_form/form.php')) {
        include (DATA_PATH . 'cache_form/form.php');
    }
