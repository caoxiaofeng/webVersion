...
elseif ($action == 'add_save') {
    ...
    if (strlen($lang_name) > 60) {
        msg('<span style="color:red">语言名称太长，请缩短</span>');
    }
    $lang_order = isset($lang_order) ? intval($lang_order) : 0;
    if (strlen($lang_order) > 60) {
        msg('<span style="color:red">语言排序太长，请缩短</span>');
    }
    if (! isset($lang_tag)) {
    ...
    if (strlen($lang_tag) > 60) {
        msg('<span style="color:red">语言标示太长，请缩短</span>');
    }
...
elseif ($action == 'search') {
    ...
    /*
     * if(empty($key)){ msg('<span style="color:red">语言变量不能为空</span>','javascript:history.go(-1);'); }
     */
    if (file_exists(DATA_PATH . 'cache/lang_cache.php')) {
        include (DATA_PATH . 'cache/lang_cache.php');
    }
...
elseif ($action == 'lang_dl') {
    ...
    if ($has_cate) {
        msg('<span style="color:red">删除失败！该语言下有内容请先删除该语言的栏目等相关内容</span>');
    }
    ...
    if (file_exists(DATA_PATH . $lang . '_info.php')) {
        @unlink(DATA_PATH . $lang . '_info.php');
    }
...
elseif ($action == 'lang_main') {
    ...
    if (! $lang_is_use) {
        msg("请先开启【{$lang_name}】语言", '?', 0);
    }
    if (! empty($id)) {
        $GLOBALS['mysql']->query("update " . DB_PRE . "lang set lang_main=0 where id=" . $id);
    }
...
elseif ($action == 'save_add_lang') {
    ...
    if (empty($lang_tag)) {
        msg('<span style="color:red">语言变量不能为空</span>');
    }
    if (! check_str($lang_tag, '/^[a-z_A-Z0-9]*$/')) {
        msg('<span style="color:red">语言变量只能是字母数字和下划线组成</span>');
    }
    
    if (! empty($lang_cache)) {
        foreach ($lang_cache as $lk => $lv) {
            if (empty($lang_value[$lv['lang_tag']])) {
                $lang_value[$lv['lang_tag']] = '没有内容';
            }
...
elseif ($action == 'save_lang_lang_edit') {
    ...
    if (strlen($lang_value) > 60) {
        msg('<span style="color:red">变量值太长，请缩短</span>');
    }
