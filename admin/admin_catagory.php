...
elseif ($action == 'add') {
    ...
    /*
     * if($parent!=0){ $p_fold=$GLOBALS['mysql']->fetch_array('select cate_fold_name from '.DB_PRE.'category where id='.$parent); if($p_fold==""){ msg('<span style="color:red">父级栏目不存在</span>','admin_catagory.php?lang='.$lang); } $cate_fold_name=$p_fold[0]['cate_fold_name'].'/'.$cate_fold_name; }
     */
    ...
    if (strlen($cate_order) > 60) {
        msg('<span style="color:red">【排列顺序】太长,请缩短</span>');
    }
