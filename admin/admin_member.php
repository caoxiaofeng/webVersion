...
elseif ($action == 'save_member_edit') {
    ...
    if (! empty($member_password) || ! empty($member_password_tow)) {
        if ($member_password != $member_password_tow) {
...
elseif ($action == 'save_member') {
    ...
    if ($is_member) {
        msg('<span style="color:red">登陆用户名【' . $member_user . '】已经存在，请更换</span>');
    }
    ...
    if ($is_mail) {
        msg('<span style="color:red">已经存在【' . $member_mail . '】邮箱，请更换</span>');
    }
    $table = DB_PRE . "member";
    $member_password = md5($member_password);
...
elseif ($action == 'save_membergroup') {
    ...
    if (strlen($member_group_name) > 60) {
        msg('<span style="color:red">会员组名称太长,请缩短</span>');
    }
    if (strlen($member_group_info) > 200) {
        msg('<span style="color:red">会员组描述太长,请缩短</span>');
    }
...
elseif ($action == 'membergroup_del') {
    ...
    $id = isset($id) ? $id : '';
    if (empty($id)) {
        msg('<span style="color:red">参数发生错误，请重新操作</span>', '?action=member_group');
    }
...
elseif ($action == 'member_del') {
    ...
    if ($id == 1) {
        msg('<span style="color:red">该会员组不能删除</span>');
    }
...
elseif ($action == 'show') {
    ...
    $id = intval($_GET['id']);
    if (empty($id)) {
        msg("<span style=\"color:red\">参数发生错误</span>");
    }
    if (file_exists(DATA_PATH . "cache/cache_member_group.php")) {
        include (DATA_PATH . "cache/cache_member_group.php");
    }
