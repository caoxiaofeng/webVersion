...
elseif ($action == 'save_flash_ad') {
    if (empty($_POST['pic'])) {
        msg('<span style="color:red">图片不能为空</span>');
    }
    if (empty($_POST['lang'])) {
        msg('<span style="color:red">参数传递错误，请重新操作</span>', '?action=flash_ad');
    }
...
elseif ($action == 'save_edit') {
    ...
    $cate_id = intval($_POST['cate_id']);
    if (empty($cate_id)) {
        msg('<span style="color:red">请选择分类</span>');
    }
    $pic = cn_substr(trim($_POST['pic']), 255);
    $pic_url = cn_substr(trim($_POST['pic_url']), 255);
    $pic_text = cn_substr(trim($_POST['pic_text']), 255);
...
elseif ($action == 'save_cate') {
    ...
    if (! empty($cate_tpl_id)) {
        ...
        if ($num[0]['n']) {
            msg('<span style="color:red">已经存在该调用id，请更改</span>');
        }
    }
...
elseif ($action == 'del_cate') {
    ...
    if ($rel[0]['n']) {
        msg('<span style="color:red">请先删除该分类下的图片</span>');
    }
