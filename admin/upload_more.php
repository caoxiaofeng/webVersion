...
if (isset($submit)) {
    ...
    $type = empty($_sys['web_upload_image']) ? array(
        'image/gif',
        'image/jpeg',
        'image/x-png',
        'image/png',
    ...
    for ($i = 0; $i <= $num; $i ++) {
        ...
        if ($_FILES['up']['size'][$i] > $size) {
            echo '第' . ($i + 1) . '张图片超过' . $size . '大小' . "【<a href=\"javascript:history.go(-1);\">返回</a>】";
            continue;
        }
        $pic_name = pathinfo($_FILES['up']['name'][$i]); // 图片信息
        $file_type = $_FILES['up']['type'][$i];
        if (! in_array(strtolower($file_type), $type)) {
            echo '第' . ($i + 1) . '张图片格式不正确' . "【<a href=\"javascript:history.go(-1);\">返回</a>】";
            continue;
        }
        $file_name = date('YmdHis') . $i . '.' . $pic_name['extension'];
        $file_path = $path . $file_name;
        unset($pic_name);
        if (! move_uploaded_file($_FILES['up']['tmp_name'][$i], $file_path)) {
            echo '第' . ($i + 1) . '张图片上传失败';
            continue;
        } else {
            $up_str .= $pic_path . $file_name . '\n';
...
<form name="up" action="?" method="post" enctype="multipart/form-data">
		...
		<p style="line-height: 25px;">
			<input name="num" value="3" id="num"
				style="width: 30px; padding: 2px 0;" />&nbsp;&nbsp;&nbsp;<input
				type="button" id="add_num" class="go" value="增加" />
		</p>
		<div id="up_p">
			<p style="line-height: 25px;">
