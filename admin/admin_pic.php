...
elseif ($action == 'save_edit') {
    ...
    if (is_uploaded_file($new_pic['tmp_name'])) {
        // 判断大小
        if ($new_pic['size'] > $_sys['upload_size']) {
            msg('图片太大，请缩小');
        }
        // 判断格式
        if (! in_array(strtolower($new_pic['type']), array(
            'image/gif',
            'image/jpeg',
            'image/png',
            'image/jpg',
    ...
    if ($is_thumb) { // 开启缩略图
        ...
        // 删除以前的缩略图
        if ($pic_thumb) {
            @unlink(CMS_PATH . 'upload/' . $pic_thumb);
        }
