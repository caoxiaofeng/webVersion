...
elseif ($action == 'save_content') {
    ...
    /*
     * $filter_str=''; if(is_array($filter)){ foreach($filter as $key=>$value){ $filter_str.=$value.','; } }else{ $filter_str=$filter; }
     */
...
elseif ($action == 'save_edit_content') {
    ...
    $is_html = ($is_html == '1') ? 1 : 0;
    /*
     * //判断表是否存在 $tables=$GLOBALS['mysql']->show_tables(); if(!in_array($table,$tables)){ msg('没有当前频道的数据表,请重新添加频道'); } $filter_str=''; if(is_array($filter)){ foreach($filter as $key=>$value){ $filter_str.=$value.','; } }else{ $filter_str=$filter; }
     */
    // 下载图片
    if ($down_file && ! empty($fields['content'])) {
        $body = $fields['content'];
