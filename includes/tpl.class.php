...
//         if (! $is_copy) {
//             $this->content = $this->content . base64_decode(return_pw());
//         }
    }
    ...
    public function del_cache()
    {
        ...
        if ($dir = @opendir($this->template_cache)) {
            while ($file = @readdir($dir)) {
                if (! is_dir($file)) {
                    @unlink($this->template_cache . $file);
                }
            }
        }
        // 清除编译文件
        if ($dir_com = @opendir($this->template_compile)) {
            while ($file_com = @readdir($dir_com)) {
                if (! is_dir($file_com)) {
                    @unlink($this->template_compile . $file_com);
                }
            }
        }
    }
    /*
     * //生成html静态页面,$tempalte-模板页;$html_name-生成文件名;$html_fold-保存目录 public function create_html($template_file,$html_name,$html_fold){ $this->template_file=$this->template_dir.$template_file.'.html'; //文件是否存在 if(!file_exists($this->template_file)){die('不存在模板文件'.$template_file.'.html');} $this->template_file_content=file_get_contents($this->template_file); $this->template_com_file=$this->template_compile.$template_file.'_compile.php'; $this->cache();//缓存文件 $this->compile();//编译模板 $this->view();//显示模板 //生成保存目录 }
     */
    // 处理模板标签ID
    ...
    {
        if (file_exists(DATA_PATH . 'cache/tpl_id_' . $this->template_lang . '.php')) {
            include (DATA_PATH . 'cache/tpl_id_' . $this->template_lang . '.php');
        }
        if (is_array($tpl_id_arr)) {
            if (in_array($tpl_id, $tpl_id_arr)) {
                return 1;
            }
        }
        return 0;
