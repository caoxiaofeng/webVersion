 ...
 * ============================================================================
*/
if ($php_file_copy != 'beescms3.0') {
    echo '<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';
} // 防止文件覆盖

/*
 * 转义函数 @param $value array || string @return array || string
 */
...
/*
 * 自动返回操作信息 $url-返回地址 $message-详细信息 return $string
 */
...
function del_cate_child($parent, $lang)
{
    ...
    if (! empty($num)) {
        msg('请先删除下级栏目');
        /*
         * foreach($num as $key=>$value){ del_cate_child($value['id'],$lang); $GLOBALS['mysql']->query("delete from ".DB_PRE."category where id=".$value['id']." and lang='".$lang."'"); }
         */
    }
    /*
     * if(file_exists(DATA_PATH.'cache_cate/cache_category'.$parent.'_'.$lang.'.php')){ unlink(DATA_PATH.'cache_cate/cache_category'.$parent.'_'.$lang.'.php'); }
     */
}
    ...
    if (! empty($field)) {
        foreach ($field as $key => $value) {
            ...
            if ($value['channel_id'] == $id) {
                if (! in_array($value['field_type'], array(
                    'checkbox',
                    'radio'
                ))) {
...
/*
 * 上传图片 $url=远程图片,$file-上传文件,$size-允许大小,$type-上传文件类型,$thumb-缩略图,$thumb_width-缩略图宽度,$mark-水印,$mark_type-水印类型,$mark_file-水印图片或文字,$mark_width-水印宽度,$mark_height-水印高度 return $arr-原始图和缩略图
 */
    ...
    if (is_uploaded_file($file['tmp_name'])) {
        ...
        if ($_sys['image_is'][0] && $logo) {
            ...
            if (! $_sys['image_type'][0]) {
                $mark_img = $php_file;
                $t_color = empty($_sys['image_text_color']) ? array(
                    "255",
                    "255",
                    "255"
                ) : explode(',', $_sys['image_text_color']);
...
function page($url = '', $page, $query = '', $totalnum, $totalpages, $cate_id = '', $ishtml = 0)
{
    ...
    if ($php_file_copy != 'beescms3.0' || $GLOBALS['tpl']->beescms_copyright != 'BEESCMS3.0') {
        echo '<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';
    } // 防止文件覆盖
    $str = "";
    $op = "";
...
function creat_html($htm_file, $err = '')
{
    ...
    if ($php_file_copy != 'beescms3.0' || $GLOBALS['tpl']->beescms_copyright != 'BEESCMS3.0') {
        $htm_data = $htm_datt . '<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';
    } // 防止文件覆盖
    ob_clean();
    $fp = fopen($htm_file, 'w');
    flock($fp, LOCK_EX);
...
function creat_html_file($htm_file, $data = '')
{
    global $php_file_copy;
    if ($php_file_copy != 'beescms3.0' || $GLOBALS['tpl']->beescms_copyright != 'BEESCMS3.0') {
        $data = $data . '<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';
    } // 防止文件覆盖
    $fp = fopen($htm_file, 'w');
    flock($fp, LOCK_EX);
    if (! fwrite($fp, $data)) {
...
/*
 * 读取缓存 return array $file--缓存文件 $arr-返回数组名
 */
...
function get_position($parent, $cate, $path, $list_php = '')
{
    ...
    if ($php_file_copy != 'beescms3.0' || $GLOBALS['tpl']->beescms_copyright != 'BEESCMS3.0') {
        $str = $str . '<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';
    } // 防止文件覆盖
    echo $str;
}
