 ...
 *
 * @access public
 * @param $cate_id:访问内容的内容ID; $table:模型表;
 *            $cat_id:栏目ID;
...
function get_content($cate_id, $table, $cat_id)
{
    ...
    if (empty($cate_id) || empty($table) || empty($cat_id)) {
        return;
    }
    $list_php = empty($channel_info['list_php']) ? 'show_list.php' : $channel_info['list_php'];
    $content_php = empty($channel_info['content_php']) ? 'show_content.php' : $channel_info['content_php'];
...
function body_pages()
{
    ...
    if ($_confing['web_html'][0]) {
        ...
        /*
         * if(!empty($rel[0]['url'])) { $url = substr($rel[0]['url'] , strrpos($rel[0]['url'] , '/')+1); $url = str_replace('.html' , '' , $url);//获取静态页面名称 }
         */
        if (! empty($content_arr_num)) { // 静态html分页
...
function weblangs($param)
{
    if (file_exists(LANG_PATH . 'lang_' . $GLOBALS['lang'] . '.php')) {
        include (LANG_PATH . 'lang_' . $GLOBALS['lang'] . '.php');
    }
    ...
    if (! empty($lang_cache)) {
        ...
        foreach ($lang_cache as $k => $v) {
            ...
            if ($v['lang_is_url']) {
                ...
                if (file_exists(DATA_PATH . $v['lang_tag'] . '_info.php')) {
                    include (DATA_PATH . $v['lang_tag'] . '_info.php');
                }
...
function nav_middle()
{
    ...
    if (! empty($cate_list)) {
        foreach ($cate_list as $k => $v) {
            $cate_nav = empty($v['cate_nav']) ? array(
                ''
            ) : explode(',', $v['cate_nav']);
...
//输出分类中的第一个文章，改自nav_middle()
function top_article($limit=5)
{
    global $lang, $cat_id, $cateid, $parent_id;
    if (file_exists(DATA_PATH . 'cache_cate/cate_list_' . $lang . '.php')) {
        include (DATA_PATH . 'cache_cate/cate_list_' . $lang . '.php');
...
//输出每个分类中的文章列表，改自nav_middle()
function category_article($limit=7)
{
    global $lang, $cat_id, $cateid, $parent_id;
    if (file_exists(DATA_PATH . 'cache_cate/cate_list_' . $lang . '.php')) {
        include (DATA_PATH . 'cache_cate/cate_list_' . $lang . '.php');
 ...
 * @access public
 * @param
 *            $table:模型表;$filter:过滤标示(如推荐等);$order:排序;$pic:是否有图片;$limit:数量
 * @return array
 */
function get_channel_content($table, $limit = '0,5', $order_type = 'id', $filter = '', $pic = 'no', $order = 'desc', $lang = '')
{
    ...
    $sql .= (in_array($order_type, array(
        'id',
        'hits',
        'addtime',
        'updatetime'
    ))) ? " GROUP BY m.id order by m.top desc, m." . $order_type : " GROUP BY m.id order by m.top desc, m.id ";
    $sql .= (in_array($order, array(
        'asc',
        'desc'
    ))) ? ' ' . $order : ' desc';
 ...
 * @access public
 * @param
 *            $cate_id:栏目id;$filter:过滤标示(如推荐等);$order:排序;$pic:是否有图片;$limit:数量
 * @return array
 */
 ...
 * @access public
 * @param
 *            $cate_id:栏目id;$filter:过滤标示(如推荐等);$order_type:排序类型;$pic:是否有图片;$limit:数量
 * @return array
 */
function get_else_content($cate_id = '', $limit = '0,5', $order_type = 'id', $filter = '', $pic = 'no', $order = 'desc', $lang = '', $like = 0)
{
    ...
    $sql .= empty($limit) ? ' limit 0, 15' : " limit " . $limit;
    $rel = $GLOBALS['mysql']->fetch_asc($sql);
    if (! empty($rel)) {
        $path = CMS_SELF;
        $i = 1;
 ...
 * @return array
 */
function get_top_content($limit = '0,5', $order_type = 'id', $filter = '', $pic = 'no', $order = 'desc', $lang = '', $like = 0)
{

    $content_php = 'article/show_article.php';//架设都是文章模块类型
    
    $sql = "select m.* from " . DB_PRE . "maintb as m where m.verify=0 ";
    $sql .= empty($filter) ? '' : " and m.filter like '%" . $filter . "%'";
    if (! empty($like)) {
        $sql .= empty($GLOBALS['relave_key']) ? '' : " and m.title like '%" . $GLOBALS['relave_key'] . "%'";
    }
 ...
 * @access public
 * @param
 *            $limit:数量;$lang:语言标示;$is_reply:过滤留言,1为回复。0为所有;$order:排序，id为id排序,addtime为时间排序
 * @return array
 */
 ...
 * @access public
 * @param
 *            $id-添加的内容ID值，$tabel-模型表，$field-表中的字段
 * @return array
 */
 ...
 * @access public
 * @param
 *            $tpl_id:模板标示id;$filter:过滤标示(如推荐等);$order_type:排序类型;$pic:是否有图片;$limit:数量
 * @return array
 */
 ...
 * @access public
 * @param
 *            $is_rand:1为随机;$filter:过滤标示(如推荐等);$order_type:排序类型;$pic:是否有图片;$limit:数量
 * @return array
 */
function get_rand_content($key = '', $limit = '0,5', $order_type = 'id', $filter = '', $pic = 'no', $order = 'desc', $lang = '')
{
    ...
    $sql .= (in_array($order_type, array(
        ...
        'addtime',
        'updatetime'
    ))) ? " GROUP BY m.id order by" . $sql_rand . " m.top desc, m." . $order_type : " GROUP BY m.id order by" . $sql_rand . " m.top desc, m.id ";
    $sql .= (in_array($order, array(
        'asc',
        'desc'
    ))) ? ' ' . $order : ' desc';
 ...
 * @access public
 * @param
 *            $cate_id:栏目id，多个用,分割;$filter:过滤标示(如推荐等);$order_type:排序类型;$pic:是否有图片;$limit:数量
 * @return array
 */
 ...
 * @access public
 * @param
 *            $filter:过滤标示(如推荐等);$order_type:排序类型;$pic:是否有图片;$limit:数量
 * @return array
 */
 ...
 * @access public
 * @param
 *            $id:下载内容ID;$field:下载字段名称;$table:模型表名称
 * @return array
 */
