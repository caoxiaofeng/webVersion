...
<title><?php echo content('title');?><?php echo 	cateinfo('cate_title_seo');?>_<?php echo webinfo('web_name');?></title>
...
foreach($fun_return as $nav){?> 
			...
			<div id="mainContent">
				...
				<div id="mainContent_article_list">
					<div class="box_in">
						<p class="title" style=""><?php echo content('title');?></p>
						<p class="info">
							<span>点击次数:<script language="javascript"
									src="<?php cmspath('includes');?>/hits.php?id=<?php echo content('id');?>"></script></span><span>更新时间:<?php echo 								date('Y-m-d H:m:s',content('updatetime'));?></span><span><a
								href="javascript:window.print()">【打印】</a></span><span><a
								href="javascript:self.close()">【关闭】</a></span>
						</p>
						<div class="arc_info"><?php echo content('info');?></div>
						<!--摘要-->
						<div class="arc_body">
							<table width='100%'>
								<tr>
...
foreach($fun_return as $v){?> <a href="<?php echo $v['url'];?>"
								target="_blank"><?php echo $v['name'];?></a> <?php 
}
}?>
						</div>
						<?php }?>
						<!-- <div class="arc_link" style="clear: both">
							<span><?php echo content('prev');?></span><span><?php echo 								content('next');?>}</span
						</div> -->
